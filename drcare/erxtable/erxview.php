...
include_once 'connect-db.php';
  ...
  $erxid = $_POST['erxid'];
  $_SESSION['erxid']=$erxid;
  $erxquery=$mysqli->query("SELECT * from erx WHERE erx_id='$erxid'");
  $erxrow=$erxquery->fetch_assoc();
  $patient_id=$erxrow['patient_id'];
  $hospital_id=$erxrow['hospital_id'];
  $patient_to =$erxrow['patient_to'];
  $patient_from=$erxrow['patient_from'];
  $medication = $erxrow['medication'];
  $sig =$erxrow['sig'];
  $dispense =$erxrow['dispense'];
  $dispense_unit =$erxrow['dispense_unit'];
  $message=$erxrow['message'];
  $doct_id=$erxrow['doctor_id'];
  ...
  */
  ...
  $patientquery= $mysqli->query("SELECT firstname, lastname, email, contact, address FROM phr WHERE user_id = '$patient_id'");
  $hospitalquery= $mysqli->query("SELECT NAME, EMAIL, ADDRESS, TELEPHONE, ZIP FROM hospital_csv WHERE ID = '$hospital_id'");
  $doctorquery= $mysqli->query("SELECT firstname, lastname, email, city, pincode, practice_type FROM doctor_entries WHERE user_id ='$doct_id'");
...
//echo "Values Inserted in Database Successfully!";
$mysqli->close();
?>
  ...
  <body>
    <nav class="navbar py-4 navbar-expand-lg ftco_navbar navbar-light bg-light flex-row">
	  ...
	  <nav class="navbar navbar-expand-lg navbar-dark bg-dark ftco-navbar-light" id="ftco-navbar">
	    <div class="container d-flex align-items-center">
	      ...
	      <p class="button-custom order-lg-last mb-0"><a href="view.php" class="btn btn-secondary py-2 px-3">Back</a></p>
    ...
    <!-- END nav -->
			...
			<div class="container">
	          ...
	          <div class="heading-section mb-5">
				 ...
				 echo"
		            ...
		            <h2 class='mb-4' style='font-size: 28px;'>Authorized Doctor: $docfname $doclname</h2>
	          ...
	          <div class="pl-md-5 ml-md-5 mb-5">
							<p>Migrating to any new town or country can be cumbersome and quite difficult for any person to adapt to the habitat and its weather, food, etc. Searching for a new doctor can be stressful-especially if you are managing a chronic condition or if a decision has to be made quickly. ReNT provides a solution to these challenges by remotely connecting to your family doctor and the doctor(s) in turn connects with the local or international hospital nurses to address the patient in need.</p>
								...
								<div class="col-lg-6">
									<div class="services-2 d-flex">
										...
										echo"
											...
											<p>'$medication'  $dispense $dispense_unit</p>" ?>
										</div>
									</div>
								</div>
								<div class="col-lg-6">
		...
		"?>							
								...
								<p class="button-custom order-lg-last mb-0"><a href="view.php" class="btn btn-secondary py-2 px-3">Back</a></p>&nbsp;
								<p class="button-custom order-lg-last mb-0"><button onclick="printFunction()" class="btn btn-secondary py-2 px-3">Print</button></p>
								
								<p class="button-custom order-lg-last mb-0"><a href="../viewNurse.php" class="btn btn-secondary py-2 px-3">Schedule Nurse</a></p><p><br></p> 
							</div>
						</div>
					</div>
				</div>
